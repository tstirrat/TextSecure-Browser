...
window.textsecure.crypto = function() {
    ...
    testing_only.privToPub = function(privKey, isIdentity) {
        ...
        if (textsecure.nacl.USE_NACL) {
            return textsecure.nacl.postNaclMessage({
                command: "bytesToPriv",
                priv: privKey
                ...
                return textsecure.nacl.postNaclMessage({
                    ...
                    return {
                        pubKey: prependVersion(message.res.slice(0, 32)),
                        privKey: priv
                    };
                });
            });
            ...
            return Promise.resolve({
                pubKey: prependVersion(toArrayBuffer(curve25519(priv))),
                privKey: privKey
            });
        }
    }
    var privToPub = function(privKey, isIdentity) {
        return testing_only.privToPub(privKey, isIdentity);
    }

    testing_only.createNewKeyPair = function(isIdentity) {
        return privToPub(getRandomBytes(32), isIdentity);
    }
    var createNewKeyPair = function(isIdentity) {
        return testing_only.createNewKeyPair(isIdentity);
    }
    ...
    crypto_storage.getStoredKeyPair = function(keyName) {
        ...
        if (res === undefined)
            return undefined;
        return {
            pubKey: toArrayBuffer(res.pubKey),
            privKey: toArrayBuffer(res.privKey)
        };
    }
    ...
    testing_only.ECDHE = function(pubKey, privKey) {
        ...
        return new Promise(function(resolve) {
            if (textsecure.nacl.USE_NACL) {
                textsecure.nacl.postNaclMessage({
                    command: "ECDHE",
                    priv: privKey,
                    pub: pubKey
                }).then(function(message) {
    ...
    var ECDHE = function(pubKey, privKey) {
        return testing_only.ECDHE(pubKey, privKey);
    }
    ...
    var calculateRatchet = function(session, remoteKey, sending) {
        ...
        return ECDHE(remoteKey, toArrayBuffer(ratchet.ephemeralKeyPair.privKey)).then(function(sharedSecret) {
            return HKDF(sharedSecret, toArrayBuffer(ratchet.rootKey), "WhisperRatchet").then(function(masterKey) {
                if (sending)
                    session[getString(ratchet.ephemeralKeyPair.pubKey)] = {
                        messageKeys: {},
                        chainKey: {
                            counter: -1,
                            key: masterKey[1]
    ...
    var removeOldChains = function(session) {
        ...
        for (var i = 0; i < session.oldRatchetList.length; i++) {
            ...
            if ((!objectContainsKeys(session[ratchet].messageKeys) && (session[ratchet].chainKey === undefined || session[ratchet].chainKey.key === undefined)) || entry.added < new Date().getTime() - MESSAGE_LOST_THRESHOLD_MS) {
                ...
                return HKDF(toArrayBuffer(chain.messageKeys[message.counter]), '', "WhisperMessageKeys").then(function(keys) {
                    ...
                    return verifyMAC(macInput.buffer, keys[1], mac).then(function() {
                        return window.textsecure.subtle.decrypt({
                            ...
                            }, keys[0], toArrayBuffer(message.ciphertext))
                            .then(function(paddedPlaintext) {
                                ...
                                if ((finalMessage.flags & textsecure.protobuf.PushMessageContent.Flags.END_SESSION) == textsecure.protobuf.PushMessageContent.Flags.END_SESSION)
    ...
    self.encryptAttachment = function(plaintext, keys, iv) {
        ...
        return window.textsecure.subtle.encrypt({
            name: "AES-CBC",
            iv: iv
        }, aes_key, plaintext).then(function(ciphertext) {
            ...
            return fillMessageKeys(chain, chain.chainKey.counter + 1).then(function() {
                return HKDF(toArrayBuffer(chain.messageKeys[chain.chainKey.counter]), '', "WhisperMessageKeys").then(function(keys) {
                    ...
                    return window.textsecure.subtle.encrypt({
                        name: "AES-CBC",
                        iv: keys[2].slice(0, 16)
                    }, keys[0], paddedPlaintext.buffer).then(function(ciphertext) {
    ...
    self.generateKeys = function() {
        ...
        if (identityKeyPair === undefined)
            return crypto_storage.getNewStoredKeyPair("identityKey", true).then(function(keyPair) {
                return identityKeyCalculated(keyPair);
            });
        else
            ...
            return ECDHE(masterEphemeral, keyPair.privKey).then(function(ecRes) {
                return HKDF(ecRes, masterEphemeral, "WhisperDeviceInit").then(function(keys) {
                    ...
                    return verifyMAC(ivAndCiphertext, ecRes[1], mac).then(function() {
                        window.textsecure.subtle.decrypt({
                            name: "AES-CBC",
                            iv: iv
                        }, ecRes[0], ciphertext).then(function(plaintext) {
